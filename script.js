document.addEventListener('DOMContentLoaded', () => {
    ...
    // Comprobar si la música ya había sido iniciada en otra página del sitio
    if (musicaFondo) {
        musicaFondo.volume = 0.6;
        if (localStorage.getItem('circoMusicaSonando') === 'true') {
            musicaFondo.play().catch(error => {
                console.warn("Autoplay bloqueado por el navegador: ", error);
            });
            // Ocultar portada de inmediato si ya se había entrado al circo
        ...
        btnEntrar.addEventListener('click', () => {
            if (musicaFondo) {
                ...
                musicaFondo.play().then(() => {
                    localStorage.setItem('circoMusicaSonando', 'true');
